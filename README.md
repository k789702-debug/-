...
> 醫事檢驗師國家考試「**醫學分子檢驗學**」的免費、開源、資料驅動複習網站。
> 每張卡片都掛**真實近年國考題（109–113 年）**，技術與臨床雙向互連。
...
**🔗 線上看：** `https://<你的帳號>.github.io/<repo>/` ｜ **共 30 張卡**（技術 17 ＋ 臨床 13）
...
- `qa` 格式 `["113年 第46題", "考點摘要"]`，至少一筆，且須為真實國考題。
- 互連：技術卡 `applications[]` 對到臨床卡 `name`；臨床卡 `method[]` 對到技術卡的 `abbr/en/zh`，名稱對齊才會自動生成連結。
